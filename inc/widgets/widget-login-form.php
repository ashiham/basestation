 ...
 *
 * @since Base Station .55
 */

/**
 * Login form widget class.
 */
class basestation_Widget_Login extends WP_Widget {
	 ...
	 * @since 0.1.0
	 */
	function basestation_Widget_Login() {

		/* Set up the widget options. */
		$widget_options = array(
			'classname' => 'login',
			'description' => esc_html__( 'A widget that allows users to log into your site.', 'basestation' )
		);
		...
		$control_options = array(
			'width' => 800,
			'height' => 350,
			'id_base' => 'basestation-widget-login'
		);

		/* Create the widget. */
		$this->WP_Widget( 'basestation-widget-login', esc_attr__( 'Login Form', 'basestation' ), $widget_options, $control_options );
	}
	...
	function widget( $args, $instance ) {
		...
		/* If the current user is not logged in. */
			...
			echo '<div class="basestation-login-form">' . basestation_wp_login_form( $args ) . '</div>';
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => esc_attr__( 'Log In', 'basestation' ),
			'label_username' => esc_attr__( 'Username', 'basestation' ),
			'label_password' => esc_attr__( 'Password', 'basestation' ),
			'label_log_in' => esc_attr__( 'Log In', 'basestation' ),
			'label_remember' => esc_attr__('Remember Me', 'basestation' ),
			'form_id' => 'loginform',
			...
			'value_username' => '',
			'show_avatar' => true,
			'logged_out_text' => esc_html__( 'Please log into the site.', 'basestation' ),
			'logged_in_text' => esc_html__( 'Logged in as', 'basestation' )
		);
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'basestation' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'remember' ); ?>"><?php _e( '"Remember me" checkbox?', 'basestation' ); ?> <code>remember</code></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'value_remember' ); ?>"><?php _e( 'Check "remember me"?', 'basestation' ); ?> <code>value_remember</code></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_avatar' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_avatar'], true ); ?> id="<?php echo $this->get_field_id( 'show_avatar' ); ?>" name="<?php echo $this->get_field_name( 'show_avatar' ); ?>" /> <?php _e('Display avatar?', 'basestation'); ?></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'logged_out_text' ); ?>"><?php _e( 'Logged out text:', 'basestation' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'logged_in_text' ); ?>"><?php _e( 'Logged in text:', 'basestation' ); ?></label>
