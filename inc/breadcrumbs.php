 ...
 */
function basestation_breadcrumbs( $args = array() ) {
	...
	if ( !empty( $trail ) && is_array( $trail ) ) {
		...
		$breadcrumb = '<ul class="'. apply_filters('basestation_breadcrumb_container_class', 'breadcrumb breadcrumbs') .'">';
...
function breadcrumb_trail_get_items( $args = array() ) {
	...
	if ( !is_front_page() && $args['show_home'] )
		$trail[] = '<a href="' . home_url() . '" title="' . esc_attr( get_bloginfo( 'name' ) ) . '" rel="home" class="trail-begin current">' . $args['show_home'] . '</a>';
